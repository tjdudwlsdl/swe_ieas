...
public class LocationUtil {
	...
	public static JSONArray getTopLocationList() {
		return getLocationList(TOP_URL);
	}
	
	// 시/군/구 선택을 위한 리스트르 가져옵니다.
	public static JSONArray getMdlLocationList(String topcode) {
		return getLocationList(String.format(MDL_URL_FORMAT, topcode));
	}
	
	// 동/읍/면 선택을 위한 리스트를 가져옵니다.
	public static JSONArray getLeafLocationList(String mdlcode) {
		return getLocationList(String.format(LEAF_URL_FORMAT, mdlcode));
	}
	
	// 주어진 URL로부터 장소 리스트를 가져옵니다.
	private static JSONArray getLocationList(String localUrl) {
